...
describe BookOrder do
  ...
  describe '#lookup_duration' do
    describe 'when not satisfied' do
      subject { BookOrder.new('book', 'person') }
      its(:lookup_duration) { should eq Float::INFINITY }
    end
    describe 'when satisfied' do
      subject { BookOrder.new('book', 'person', Time.now - 600, Time.now) }
      its(:lookup_duration) { should eq 600 }
    end
  end
